...
void init_list(Table_pointer tp)
{
	for(int i=0;i<tp->num_column-1;i++)//i loop through each item in table
	{
		string s = "";
		stringstream out;
		out << tp->index[i];//get the string version of index
		s = out.str();
	...
	while(lt != table_list.end())
	{
		...
		for(int i=0;i<lt->num_column;i++){
			if(lt->support[i] >= threshold){
				string s = "";
				stringstream out;
				out << lt->index[i];//get the string version of index
				s = out.str();
				cout<<"  "<<lt->str_item+" "+s<<" ["<<((lt->count<lt->support[i])?lt->count:lt->support[i])<<"]"<<endl;
			}
		}
...
void main(int argc, char *argv[])
{
 ...
 /* output 1-itemsets here */
 cout<<"\nLarge 1-itemsets[support]:"<<endl;
 for(int i=0;i<tp->num_column;i++)
	cout<<"  "<<tp->index[i]<<" ["<<tp->support[i]<<"]"<<endl;
 cout<<"\nLarge n-itemsets[support]:"<<endl;
